    ...
    script.onload = () => {
        window.kakao.maps.load(() => {
            ...
            const createMap = async () => {
                ...
                const getGasStationData = async () => {
                    // 승강기 데이터를 가져와 필요한 데이터를 삽입합니다
                    // 1. 매핑된 데이터를 가져옵니다
                    const response = await axios.get("http://10.41.222.46:8080/transport/gas");
                    ...
                    for (let i = 0; i < positions.length; i++) {
                        ...
                        let iwContent = `<div style="width:350px;text-align:center;padding:10px;font-family: 'NanumGothic';">
                            <div>${obj.업소명}</div>
                            <div>${obj.소재지}</div>
                            <div>${obj.전화번호}</div>
