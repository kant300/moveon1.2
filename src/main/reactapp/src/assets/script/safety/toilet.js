        ...
        window.kakao.maps.load(() => {
            ...
            const createMap = async () => {
                ...
                const getToiletData = async () => {
                    ...
                    // 1. 매핑된 데이터를 가져옵니다
                    const response = await axios.get("http://10.41.222.46:8080/safety/toilet");
                    ...
                    for (let i = 0; i < positions.length; i++) {
                        ...
                        let iwContent = `<div style="width:400px;text-align:center;padding:10px;font-family: 'NanumGothic';">
                            <div>${obj.화장실명}</div>
                            <div>${obj.소재지도로명주소}</div>
                            <div>관리기관: ${obj.관리기관명} ${obj.전화번호}</div>
                            <div>개방시간: ${obj.개방시간상세}</div>
