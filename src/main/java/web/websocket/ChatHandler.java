...
public class ChatHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        // 클아이언트 정보
        String cno = (String) session.getAttributes("cno"); // 접속 채팅방 번호
        String mno = (String) session.getAttributes("mno"); // 접속 회원 번호
        String mname; // 회원 이름
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        Map<String , String > mag = objectMapper.readValue( message.getPayload(), Map.class ); // type 별로 분리 시켜줘  ex) 챗 1 , 내용 : 안녕 , 이름 : 길동
        if (mag.get("TYPE").equals("join")) { // mag 정보를 type 별로 받아 합체(join) 해서 보내기/받기
            String mmessage = mag.get("mmessage"); // 메세지 내역
            String mno = mag.get("mno"); // 회원번호
            String mname; // 회원 이름

            String cno = mag.get("cno"); // 채팅방 정보 호출(꺼내기)
            ...
            if (mag.containsKey(player)){ // 만약에 방번호가 존재하면
        ...
        } else if (mag.get("TYPE").equals("mag")) {
    ...
    public void alarmMessage( String cno , String mmessage) throws Exception{
        ...
        // 예외 던지기
        // map 보내고자 하는 정보 작성
        Map< String , String > mag = new HashMap<>();
        mag.put("type" , "alarm");
        mag.put("message" , mmessage);
        // map 타입 json 으로 변환
        String jmag =objectMapper.writeValueAsString(mag);
        // 같은방 모든 메세지 내용 알림 보내기
        for (WebSocketSession session : player.get(cno)){
            session.sendMessage( new TextMessage( jmag ));
        }
    } // alarmMessage end
